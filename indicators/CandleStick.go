...
func (cs *CandleStick) IsDownTrend() bool {
	...
	BlackCount = 0
	if cs.DataSet[:cs.LastIndex-1].NewCandleStick().IsBlack() {
		BlackCount++
	}
	if cs.DataSet[:cs.LastIndex-2].NewCandleStick().IsBlack() {
		BlackCount++
	}
	if cs.DataSet[:cs.LastIndex-3].NewCandleStick().IsBlack() {
		BlackCount++
	}
...
func (cs *CandleStick) IsUpTrend() bool {
	...
	WhiteCount = 0
	if cs.DataSet[:cs.LastIndex-1].NewCandleStick().IsWhite() {
		WhiteCount++
	}
	if cs.DataSet[:cs.LastIndex-2].NewCandleStick().IsWhite() {
		WhiteCount++
	}
	if cs.DataSet[:cs.LastIndex-3].NewCandleStick().IsWhite() {
		WhiteCount++
	}
...
func (cs *CandleStick) IsMaruboza() bool {
	...
	if cs.IsFuzzyEqual(cs.LastData.Open, cs.LastData.Low) && cs.IsFuzzyEqual(cs.LastData.Close, cs.LastData.High) {
		return true
	}
...
func (cs *CandleStick) IsHammer() bool {
	...
	if cs.IsFuzzyEqual(cs.LastData.Close, cs.LastData.High) || cs.IsFuzzyEqual(cs.LastData.Open, cs.LastData.High) == false {
		return false
	}
...
func (cs *CandleStick) IsShootingStar() bool {
	...
	if cs.IsFuzzyEqual(cs.LastData.Close, cs.LastData.Low) || cs.IsFuzzyEqual(cs.LastData.Open, cs.LastData.Low) == false {
		return false
	}
...
func (cs *CandleStick) IsThreeWhiteSoldiers() bool {
	...
	if cs.IsWhite() && cs.DataSet[:cs.LastIndex-1].NewCandleStick().IsWhite() && cs.DataSet[:cs.LastIndex-2].NewCandleStick().IsWhite() == false {
		return false
	}

	// Her üçü de vücut ağırlıklı olmalı
	if cs.IsFuzzyEqual(cs.LastData.Open, cs.LastData.Low) && cs.IsFuzzyEqual(cs.LastData.Close, cs.LastData.High) == false {
		return false
	}
	if cs.IsFuzzyEqual(cs.PrevData.Open, cs.PrevData.Low) && cs.IsFuzzyEqual(cs.PrevData.Close, cs.PrevData.High) == false {
		return false
	}
	if cs.IsFuzzyEqual(cs.DataSet[cs.LastIndex-2].Open, cs.DataSet[cs.LastIndex-2].Low) && cs.IsFuzzyEqual(cs.DataSet[cs.LastIndex-2].Close, cs.DataSet[cs.LastIndex-2].High) == false {
		return false
	}

	// Sağdan sola boyutları küçülerek gitmeli
	if math.Abs(cs.LastData.Open-cs.LastData.Close) > math.Abs(cs.PrevData.Open-cs.PrevData.Close) == false {
		return false
	}
	if math.Abs(cs.PrevData.Open-cs.PrevData.Close) > math.Abs(cs.DataSet[cs.LastIndex-2].Open-cs.DataSet[cs.LastIndex-2].Close) == false {
		return false
	}

	// Sağdan sola kapanışlar azalarak gitmeli
	if cs.LastData.Close > cs.PrevData.Close && cs.PrevData.Close > cs.DataSet[cs.LastIndex-2].Close == false {
		return false
	}

	// Sağdan sola açılışlar azalarak gitmeli
	if cs.LastData.Open > cs.PrevData.Open && cs.PrevData.Open > cs.DataSet[cs.LastIndex-2].Open == false {
		return false
	}
